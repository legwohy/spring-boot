...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable
{
    ...
    final Node<K, V>[] resize()
    {
        ...
        if (oldCap > 0)
        {
            ...
            {
                threshold = Integer.MAX_VALUE;// 1 << 31
                return oldTab;
        ...
        {   // zero initial threshold signifies using defaults
            newCap = DEFAULT_INITIAL_CAPACITY;// 1 << 4  8
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);// 6
        }
        ...
        if (oldTab != null)
        {
            // 把每个bucket都移动到新的buckets中 外层数组 内层链表或红黑树
            for (int j = 0; j < oldCap; ++j)
            {
                Node<K, V> e;
                // 当前位置n为 null 扩容后的位置n+oldCap 也为null
                if ((e = oldTab[j]) != null)
                {
                    oldTab[j] = null;
                    // 下一节点 为空 直接移到bucket中
                    ...
                    {
                        ((TreeNode<K, V>)e).split(this, newTab, j, oldCap);// 切割
                    }
                    ...
                    else
                    {
                        // jdk 1.7会发生倒置
                        // 扩容后的低位(前半节) 高位(后半节)
                        Node<K, V> loHead = null, loTail = null;// 元素保持原位置(原下标的位置) 低位
                        Node<K, V> hiHead = null, hiTail = null;// 元素移到新位置(原下标的位置+原数组的长度) 高位
                        Node<K, V> next;
                        ...
                        {
                            next = e.next;
                            // (e.hash & oldCap) 判断元素是否要移位
                            // (e.hash &; (oldCap-1)) 元素的下标位置
                            if ((e.hash & oldCap) == 0)
                            {
                                // 单向链表 尾为空 直接赋值给头部 每一次在尾部插入
                                if (loTail == null) {
                                    loHead = e;
                                }else {
                                    loTail.next = e;
    ...
    final void treeifyBin(Node<K, V>[] tab, int hash)
    {
        int n, index;
        Node<K, V> e;
        // 元素为空或数组长度小于64 初始化或扩容
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY) // 1 << 6
        {
            resize(); // 初始化或扩容
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V>
    {
        ...
        TreeNode<K, V> prev;    // 是双向链表的前一个节点 还包括 next
        boolean red;// root 为black
            ...
            for (TreeNode<K,V> x = this, next; x != null; x = next)
            {
                ...
                // 后面进入循环走的逻辑，x 指向树中的某个节点
                    ...
                    for (TreeNode<K,V> p = root;;) {
                        ...
                        if ((ph = p.hash) > h)
                        {
                            dir = -1; // 当比较节点的哈希值比 x 大时， dir 为 -1
                        }
                        else if (ph < h)
                        {
                            dir = 1;// 哈希值比 x 小时 dir 为 1
                        }
                        // 当前类为空
                        else if (
