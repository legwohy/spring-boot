package com.cobra.spring.myMVC.dispatcher;

import com.cobra.spring.myMVC.annotation.*;
import com.cobra.spring.myMVC.controller.XUserController;
...
@WebServlet(name = "xDispatcherServlet",
        urlPatterns = "/*",
        loadOnStartup = 1,
        initParams = {@WebInitParam(name = "base-package",value = "com.cobra.spring.myMVC")})
public class XDispatcherServlet extends HttpServlet {
    ...
    /**
     *
     * com.cobra.spring.myMVC 下面的所有类
     * com.cobra.spring.myMVC
     *               .service
     *               .dao
     *
     */
    private void scanBasePackage(String basePackage) {
        ...
        for (File file : childrenFile){
            if(file.isDirectory()){
                // 目录 继续往下查找 com.cobra.spring.myMVC.service
                scanBasePackage(basePackage+"."+file.getName());
            }else if(file.isFile()){
                // 只读取到名字 com.cobra.spring.myMVC.service.UserServiceImpl.class 去掉class
