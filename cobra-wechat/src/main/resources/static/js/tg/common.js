...
function getSourceUrl(){
    ...
    if( curUrl != window.location.href ){
        curUrl && window.localStorage.setItem("sourceUrl",curUrl);
        window.localStorage.setItem("curUrl",window.location.href);
    }
    return window.localStorage.getItem("sourceUrl") ? window.localStorage.getItem("sourceUrl") : '/';
}
...
function inArray(obj,arr){
    var len = arr.length;
    if(getTypeOf(arr) != "[object Array]") throw "arguments is not Array";
    for (var v = 0; v < len; v++){
        if (obj == arr[v]) return true;
    }
    return false;
}
...
function showMask(cName, bgColor, opacityVal){
    ...
    layer.id = 'mask';
    layer.style.cssText = "position:fixed; z-index:100; top:0; "+( isIE(7) ? 'left:0;' : '')+" width:100%; height:100%; background-color:"+bgColor+"; opacity:"+(opacityVal/100)+"; filter:alpha(opacity="+opacityVal+"); -moz-opacity:"+(opacityVal/100)+"; -khtml-opacity:"+(opacityVal/100)+";";
...
function showExDialog(content, btn1, func1, btn2, func2, pTop, cName){
    ...
    var str = '';
    str += '<div style="position: fixed;z-index:200;top:'+pTop+';width:80%;max-width:380px;background-color:#fff;border-radius:5px;-moz-border-radius:5px 5px 5px 5px;-webkit-border-radius:5px;">';
    str += '<div class="_content" style="text-align:center;margin:12% auto;padding:0 10%;letter-spacing:1px;">'+content+'</div>';
    if(arguments[1]) str += '<div class="_btn" style="display:inline-block;width: '+((arguments[1] && arguments[3]) ? '35%' : '80%')+';padding: 2.5% 0;margin-left: 10%;margin-bottom: 4%;background-color: #61cae4;color: #fff;text-align: center;border-radius: 4px;-moz-border-radius: 4px 4px 4px 4px;-webkit-border-radius: 4px;" onclick="'+func1+'">'+btn1+'</div>';
...
function showCustomExDialog(content, pTop, boxWidth, cName){
    ...
    document.body.appendChild(dialogWraper);
    var str = '';
    str += '<div style="position: fixed;z-index:200;top:'+pTop+';width:'+boxWidth+';background-color:#fff;">';
